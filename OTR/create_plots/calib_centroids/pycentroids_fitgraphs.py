...
#    magF.close()
    #Compute mag v2
    xl, xh, yl, yh = Double(0), Double(0), Double(0), Double(0)
    hor.GetPoint(2,xl,yl)
    hor.GetPoint(3,xh,yh)
    magHor = np.abs(xh-xl)
    ver.GetPoint(2,xl,yl)
    ver.GetPoint(3,xh,yh)
    magVer = np.abs(yh-yl)
    ...
    f.close()
    #"Data"
    files = 'output/F{}_data.txt'
    f = open(files.format(n), 'w')
    #Maybe at some point change where parameters are located - this is ugly
    ...
    tht = pms_split[2]
    parameters.close()
    #f.write(f"{tht} {slope} {k} {magHor} {magVer}\n")
    f.write(f"{tht} {slope}\n")
    f.close()

...
with open('output/%s3.txt'%(name_phi))     as f:
    phi   = [ [float(item)     for item in lines.split()] for lines in f]

with open('output/%s3.txt'%(name_theta))   as f:
    theta = [ [float(item)     for item in lines.split()] for lines in f]

with open('output/%s_3.txt'%(name_boo_M1)) as f:
...
h2 = ax2.hist(list_theta0, bins = 500)
h3 = ax3.hist(list_phi0,   bins = 500)

print(h2[1])
ax2.set_xlabel("$\\theta$")
ax2.axvline(0.04,  c = 'r', ls = '--')
ax2.axvline(6.24,  c = 'r', ls = '--')
ax3.axvline(0.758, c = 'r', ls = '--')
ax3.axvline(0.813, c = 'r', ls = '--')
ax3.set_xlabel("$\\phi$")
ax2.set_yscale('log')
ax3.set_yscale('log')
